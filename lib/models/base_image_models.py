...
class BaseClassificationModel(BaseModel):
    ...
    def __init__(
        ...
        self.bin_values = Tensor(bins[:-1] + (bins[1:] - bins[:-1]) / 2).cuda()
    ...
    def monitor_metrics(self, prediction: Tensor, targets: Optional[Tensor]=None) -> dict:
        ...
        if type(targets) == Tensor:
            ...
            bin_prediction = argmax(prediction,dim=1)

            numeric_prediction = 100*self.bin_values[bin_prediction]
            numeric_targets = 100*self.bin_values[targets]

            sum_squared_error = nn.MSELoss(reduction="sum")(numeric_prediction,numeric_targets).cpu().numpy()
            root_mean_squared_error = sum_squared_error.sum()**(0.5)
            #standard_error = (sum_squared_error.sqrt() / (prediction.shape[0] - 2))

            metrics = {
                "binary_multiclass_loss": binary_multiclass_loss.cpu().numpy(),
                "cross_entropy_loss": cross_entropy_loss.cpu().numpy(),
                "root_mean_squared_error": root_mean_squared_error,
                #"standard_error": standard_error.numpy()
            }
        else:
            metrics = {
    ...
    def forward(
        ...
        #pawpularity_estimate = nn.functional.sigmoid(output)
        pawpularity_estimate = output
